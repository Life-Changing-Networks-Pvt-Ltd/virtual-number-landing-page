...
type DemoBookingProps = {
  ...
  source?: DemoBookingPayload["source"];
};

export default function DemoBooking({ onClose, interestedPlan = "", source = "navbar" }: DemoBookingProps) {
  ...
  const [details, setDetails] = useState({ company: "", name: "", designation: "", message: "", email: "", phone: "", purpose: [] as string[] });
  const [purposeError, setPurposeError] = useState(false);
  const [submitting, setSubmitting] = useState(false);
  const [bookedMeetLink, setBookedMeetLink] = useState("");
  ...
  return (
    <dialog ref={dialog} className="demo-booking" aria-labelledby="demo-heading" onCancel={onClose} onClick={(event) => { if (event.target === event.currentTarget) { const rect = event.currentTarget.getBoundingClientRect(); if (event.clientX < rect.left || event.clientX > rect.right || event.clientY < rect.top || event.clientY > rect.bottom) onClose(); } }}>
      ...
      <div className="demo-header">
        <span className="demo-eyebrow">SELLERSLOGIN · {step === 2 ? "DEMO BOOKED" : `STEP ${step + 1} OF 2`}</span>
        <h2 id="demo-heading" ref={heading} tabIndex={-1}>{step === 0 ? "Book a Demo" : step === 1 ? "Enter Your Contact Details" : "Your demo is booked"}</h2>
        <p className="demo-intro">{step === 0 ? "Choose a date and time that works for you." : step === 1 ? "Share your contact details and select how you’d like to use your AI agent." : "Thank you. Our team will contact you shortly."}</p>
      ...
      {step === 0 ? <>
        ...
        <div className="demo-actions">
          <small> </small>
          <button className="demo-primary" type="button" disabled={!validSlot} onClick={() => { if (validSlot) setStep(1); else setError("Please choose a future time slot."); }}>Continue →</button></div>
      </> : step === 1 ? <>
        <form onSubmit={async (event) => {
          event.preventDefault();
          if (submitting) return;
          if (!validSlot) { setError("That time has passed. Please choose a new slot."); setStep(0); return; }
          if (!details.name.trim()) { setError("Please enter your name."); return; }
          if (!designations.includes(details.designation)) { setError("Please select your designation."); return; }
          if (!details.company.trim()) { setError("Please enter your company name."); return; }
          if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(details.email.trim())) { setError("Please enter a valid email address."); return; }
          if (!/^[6-9]\d{9}$/.test(details.phone)) { setError("Please enter a valid 10-digit Indian mobile number."); return; }
          if (!details.purpose.length) { setPurposeError(true); purposeTrigger.current?.focus(); return; }
          if (!details.message.trim()) { setError("Please describe how you would like your AI agent to work."); return; }
          setError(""); setSubmitting(true);
          try {
            const result = await submitDemoBooking({ companyName: details.company.trim(), name: details.name.trim(), designation: details.designation, message: details.message.trim(), email: details.email.trim(), phone: details.phone, purposes: details.purpose, demoDate: date, demoSlot: slots[slot!].label, interestedPlan, source });
            setBookedMeetLink(result.data?.meetLink || "");
            setEmailQueued(result.data?.notificationStatus === "queued");
            setStep(2);
          } catch (submissionError) {
            setError(submissionError instanceof Error ? submissionError.message : "We could not book your demo. Please try again.");
          ...
          <div className="demo-scroll">
            ...
            <div className="demo-fields">
              ...
              <label><strong>Designation <span aria-hidden="true">*</span></strong><select name="designation" autoComplete="organization-title" required value={details.designation} onChange={e => setDetails({ ...details, designation: e.target.value })}><option value="" disabled>Select designation</option>{designations.map(designation => <option key={designation} value={designation}>{designation}</option>)}</select></label>
              <label><strong>Company Name <span aria-hidden="true">*</span></strong><input name="company" autoComplete="organization" required maxLength={150} value={details.company} onChange={e => setDetails({ ...details, company: e.target.value })} /></label>
              ...
              <label className="demo-full"><strong>Mobile Number <span aria-hidden="true">*</span></strong><span className="demo-phone"><span className="demo-phone-prefix" aria-hidden="true">+91</span><input name="phone" type="tel" inputMode="numeric" autoComplete="tel-national" required pattern="[6-9][0-9]{9}" placeholder="9876543210" title="Enter a 10-digit Indian mobile number starting with 6, 7, 8 or 9" value={details.phone} onChange={e => { let digits = e.target.value.replace(/\D/g, ""); if (digits.length === 12 && digits.startsWith("91")) digits = digits.slice(2); setDetails({ ...details, phone: digits.slice(0, 10) }); }} /></span>
              ...
              <label className="demo-full"><strong>Message <span aria-hidden="true">*</span></strong><textarea name="message" required maxLength={2000} rows={4} placeholder="Describe how you would like your AI agent to work and which tasks it should handle." value={details.message} onChange={e => setDetails({ ...details, message: e.target.value })} /></label>
            </div>
            {error && <p role="alert" className="demo-error">{error}</p>}
            <p className="demo-note">Your details will only be used to arrange this demo.</p>
          </div>
